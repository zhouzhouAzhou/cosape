...
void men_init(FILE * bin_fp)
{
    ...
    for(; imem_adrress_reg<mem_counter; imem_adrress_reg++)
    {
        ...
        printf("%d  %d\n",imem_adrress_reg,Memory[imem_adrress_reg]);
...
void CPU_run(FILE * bin_fp)
{
    ...
    while(PC_reg<(mem_counter+instruction_counter))
    {
        ...
        printf("指令___ %s\n",str_inst);
       // printf("指令___ %s\n instruction:%lld\n",str_inst,instruction);
        /*
        指令译码
        */
        // 0xffff是十进制的65535,2^16=65536  1 0000 0000 0000 0000 0000 0000
        ...
        /*
        itoaPlus(str_inst,instruction>>56,2);
        printf("Opcode指令___ %s\n\n",str_inst);
        itoaPlus(str_inst,Result_num,2);
        //printf("Result_num: %d\n",Result_num);
        printf("Result_num指令___ %s\n\n",str_inst);
        itoaPlus(str_inst,Source_num1,2);
        printf("Source_num1指令___ %s\n\n",str_inst); */
        itoaPlus(str_inst,instruction>>8,2);
        printf("Source_num2指令___ %s\n\n",str_inst);

        // int Source_num2 = (instruction>>8);
        //printf("(0>>8)&0xff:%d\n",(0>>8)&0xff);
        printf("Opcode:%d Result_num:%d Source_num1:%d Source_num2=%d\n",Opcode,Result_num,Source_num1,Source_num2);
