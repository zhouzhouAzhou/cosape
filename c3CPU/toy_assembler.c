    ...
    while(iline <line_total-1)
    {
        ...
        if(seg_flag==1)  //汇编数据段
        {
           ...
           // printf("line_split %s %s %d\n",revbuf[0],revbuf[1],num);
            ...
            //分配地址，并写值
            long long var_tmep;//long long占8个字节
        ...
        if(seg_flag==2)  //汇编代码段
        {
            if(strstr(asm_line,"LOAD") != NULL)
            {
                ...
                /*else
                {
                    temp_instruct=((((long long)opcode<<16) | (long long)2)<<16  | (long long)temp_mem)<<24;
                    iload =0;
                }*/
            }
