...
double clockperiods = 0,clockperiods1 = 0;//写的文件的时钟周期数差之和,初始化为零
...
void save(char filename[],int size)
{
    ...
    int i;
    for(i =0; i<size; i++) //循序写size1次kb大小的数据
    {
        fputs(str,fp);
    }
    fclose(fp);//关闭输出文件
...
void read(char filename[])
{
    ...
    fclose(fp);//关闭输入文件
    QueryPerformanceCounter(&stop_t);
    clockperiods1+=(stop_t.QuadPart-start_t.QuadPart);//累加每一次读取的文件的时钟周期数差
}
...
int main()
{
    ...
    //printf("请输入文件大小，\n");
    //scanf("%d",&filesize);
    printf("写文件大小(KB),写文件数度(MB/s),读文件数度（MB/s)\n");
    while(filesize<=MAXFileSize) {
        ...
        //printf("%d个%dkb文件生成成功！\n",filenums,filesize);
        clockperiods1=0;
        for(i =0 ; i<filenums; i++) //循环读取filenums次文件
       {
            char chars1[20];//文件后半部分的名字
            itoa(i,chars1,10);
            char filename[100] = "E:\\实验2学生\\coasepv2\\coasep\\c5disk\\temporary_files\\";//修改为本机目录
